...
export default function OnboardingPage() {
  ...
  // Step 3: Service preferences (which services apply)
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // Create reminders for enabled services
    ...
    <div style={{ minHeight: '100vh', background: '#0f0f0f', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '48px 24px 24px', textAlign: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 8 }}>
          <Car size={24} color="#ff6b2b" />
          <span style={{ fontSize: 22, fontWeight: 700, color: '#f5f5f5' }}>CarSync</span>
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginBottom: 32 }}>
        {[1,2,3].map(s => (
          <div key={s} style={{
            ...
            background: s <= step ? '#ff6b2b' : '#2a2a2a',
            transition: 'all 0.3s',
          }} />
        ))}
      </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  ...
                  style={{
                    padding: '0 16px', background: vin.length === 17 ? '#ff6b2b' : '#1e1e1e',
                    border: '1px solid #2a2a2a', borderRadius: 10, color: 'white',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{
              ...
              <p style={{ color: '#888', fontSize: 13 }}>
                This helps us calculate accurate due dates for all your services.
              </p>
            </div>
            ...
            <button
              ...
              style={{
                background: skipLastService ? '#ff6b2b22' : 'transparent',
                border: `1px solid ${skipLastService ? '#ff6b2b' : '#2a2a2a'}`,
                borderRadius: 12, padding: '14px 16px', color: skipLastService ? '#ff6b2b' : '#888',
              ...
              {DEFAULT_SERVICES.map((s, i) => (
                <button
                  ...
                  style={{
                    ...
                    background: enabledServices.has(i) ? '#1a1a1a' : '#111',
                    border: `1px solid ${enabledServices.has(i) ? '#2a2a2a' : '#1a1a1a'}`,
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{
                      ...
                      background: enabledServices.has(i) ? '#ff6b2b' : '#2a2a2a',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      flexShrink: 0,
                    }}>
                      {enabledServices.has(i) && <CheckCircle size={12} color="white" />}
                    </div>
                    <span style={{ fontSize: 14, color: enabledServices.has(i) ? '#f5f5f5' : '#555' }}>
                      {s.name}
                    </span>
                  </div>
                  {s.is_critical && (
                    <span style={{ fontSize: 11, color: '#ef4444', background: '#ef444422', padding: '2px 8px', borderRadius: 20 }}>
                      Critical
                    </span>
                  )}
                </button>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          {step > 1 && (
            <button
              ...
              style={{
                flex: 1, padding: '16px', background: '#1a1a1a', border: '1px solid #2a2a2a',
                borderRadius: 14, color: '#f5f5f5', cursor: 'pointer', fontWeight: 600,
          ...
          <button
            ...
            disabled={saving}
            style={{
              flex: 3, padding: '16px', background: '#ff6b2b', border: 'none',
              borderRadius: 14, color: 'white', cursor: 'pointer', fontWeight: 600, fontSize: 15,
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
